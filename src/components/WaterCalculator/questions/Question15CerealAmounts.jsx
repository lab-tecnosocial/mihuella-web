...
const CEREAL_METADATA = {
  arroz: { label: "Arroz", image: "/img/rice.webp" },
  fideo: { label: "Fideo", image: "/img/pasta.webp" },
  avena: { label: "Avena", image: "/img/avena.webp" },
  quinua: { label: "Quinua", image: "/img/quinua.webp" },
  pan: { label: "Pan", image: "/img/bread.webp" },
  maiz: { label: "Maíz", image: "/img/corn.webp" },
  otro: { label: "Otro", image: "/img/three_dots.webp" },
};

export function Question15CerealAmounts({
  onNext,
  onBack,
  onExit,
  selectedCereals = [], // Cereales seleccionados en la pregunta 14
  initialValue = {},
}) {
  // Filtramos "ninguno"
  const activeCereals = selectedCereals.filter((id) => id !== "ninguno");

  const [amounts, setAmounts] = useState(() => {
    const initial = {};
    activeCereals.forEach((id) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center w-full max-w-md mx-auto">
        ...
        {/* LISTADO DINÁMICO DE CEREALES SELECCIONADOS */}
        <div className="w-full flex flex-col gap-4 mb-6">
          {activeCereals.map((id) => {
            const meta = CEREAL_METADATA[id] || {
              label: id,
              image: "/img/otro.webp",
            };
